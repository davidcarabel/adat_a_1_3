...
public class Principal {
	...
	/**
	 * Método que permite presentar las opciones del sistema. solicita el ingreso de
	 * un numero y se envia a su correspondiente proceso
	 */
	private void verMenu() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Error en el ingreso de Datos, " + "solo se permiten valores númericos",
	...
	/**
	 * Permite determinar que accion ejecutar dependiendo del parametro de ingreso
	 * correspondiente a las opciones del sistema
	 * 
	 * @param seleccion
	 */
	 ...
	 * una ventana de ingreso y se almacenan en un arreglo con toda la informacion
	 * usando para esto un ciclo for, posteriormente estos datos son almacenados en
	 * el atributo correspondiente del objeto persona para ser enviado al metodo de
	 * registro en la clase DAO
	 */
	private void registrarPersona() {
		...
		String datosSolicitados[] = { "Documento : ", "Nombre : ", "Edad: ", "Profesión: ", "Telefono: " };
		...
		for (int i = 0; i < datosSolicitados.length; i++) {
			...
			datosPersona[i] = JOptionPane.showInputDialog(null, mensajeIngreso + datosSolicitados[i], "Datos Persona",
	 ...
	 * lista se encuentra vacia quiere decir que no hay personas registradas acto
	 * seguido se presenta un mensaje en pantalla, sino se imprime la lista de todas
	 * las personas registradas en la BD
	 */
	private void obtenerRegistros() {
		...
		if (listaPersonas.size() > 0) {
			int numeroPersona = 0;
			// se recorre la lista de personas asignandose cada posicion en un objeto
			// persona
	...
	private void buscarPersona(int documento) {
		...
		if (personasEncontrada.size() > 0) {
			// se recorre la lista y se asignan los datos al objeto para imprimir los
			// valores
